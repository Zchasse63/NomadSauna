...
console.log('Creating optimized Open Graph image with No Bg logo...');

// Define paths
const ogDir = path.join(__dirname, 'public', 'images', 'og');
const sourcePath = path.join(__dirname, 'Link Picture No Bg.png'); // Use the Link Picture No Bg from root directory
const publicPath = path.join(__dirname, 'public', 'Link Picture No Bg.png'); // Destination in public directory
...
const WARM_TAUPE_RGB = { r: 217, g: 196, b: 163, alpha: 1 }; // RGB values for Warm Taupe
...
// Copy the source file to public directory if it doesn't exist there
if (!fs.existsSync(publicPath) && fs.existsSync(sourcePath)) {
  try {
    fs.copyFileSync(sourcePath, publicPath);
    console.log(`✅ Copied ${sourcePath} to ${publicPath}`);
  } catch (err) {
    console.error(`❌ Error copying file: ${err.message}`);
    // Continue with the source file
  }
}

// Use the file from the root directory if it exists, otherwise try the public directory
const linkPicturePath = fs.existsSync(sourcePath) ? sourcePath : publicPath;
...
sharp(linkPicturePath)
  .metadata()
  .then(metadata => {
    console.log(`Link Picture No Bg dimensions: ${metadata.width}×${metadata.height}`);
    ...
    return sharp(linkPicturePath)
      .resize({
        ...
        height: 630,
        fit: 'contain',
        background: WARM_TAUPE_RGB // Warm Taupe color (#D9C4A3) for consistent background
      })
      .toFile(outputPath);
  })
  .then(() => {
